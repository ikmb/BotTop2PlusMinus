...
uint32_t  CTwoBit::getChromSize(const char* chrom)
{
    map<string,chrom_data>::const_iterator iter = m_data.find(chrom);
    if(iter == m_data.end())
        return 0;
    return iter->second.dnaSize;
}
...
string CTwoBit::getFaSequence(const char* chrom,uint32_t start, uint32_t end,const char* faHeader ,uint32_t lineBreaks)
{
...
string CTwoBit::getFaSequence(const char* chrom,uint32_t start, uint32_t end,uint32_t lineBreaks)
{
    if(lineBreaks==0)
...
string CTwoBit::getSequence(const char* chrom)
{
    map<string,chrom_data>::const_iterator iter = m_data.find(chrom);
    if(iter == m_data.end())
        return "";
...
string CTwoBit::getSequence(const char* chrom,uint32_t start, uint32_t end, uint16_t flanks)
{
    map<string,chrom_data>::const_iterator iter = m_data.find(chrom);
    if(iter == m_data.end())
        return "";
...
string CTwoBit::getSequence(const char* chrom,uint32_t start, uint32_t end)
{
